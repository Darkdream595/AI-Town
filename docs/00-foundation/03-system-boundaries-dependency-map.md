...
- `RULE-FOUNDATION-010`：Domain 层 direct dependency 必须严格属于第 5.1 节的穷举集合，并与第 5.2 节的边完全一致；需要反向数据或双向业务协作时，由 Application Orchestrator 查询 owner 后映射为调用方自有的不可变 projection，或使用 Command/Event 解耦，禁止增加反向 import。
...
| 序 | Domain / 前缀 | Canonical ownership | 允许 direct provider（穷举） |
|---:|---|---|---|
...
| 5 | AI 决策与模型编排 `AI` | DecisionContext、Prompt、ActionProposal、模型路由、Utility AI | `FOUNDATION`；业务数据只能作为 Orchestrator 构造的 AI-owned read projection 输入 |
...
| 14 | 后端、API 与安全 `BACKEND` | REST/WebSocket 协议、Session、Envelope、事务编排、错误码、本地安全 | `FOUNDATION`, `WORLD`, `MAP`, `RENDER`, `RESIDENT`, `AI`, `MEMORY`, `TIME`, `PLAYER`, `DIALOGUE`, `ECON`, `MAGIC`, `COMBAT`, `EVENT` |
| 15 | 存档、启动与发布质量 `RELEASE` | SQLite 布局、Migration、Snapshot、存档、Launcher、Package、质量 Gate | `FOUNDATION`, `WORLD`, `MAP`, `RENDER`, `RESIDENT`, `AI`, `MEMORY`, `TIME`, `PLAYER`, `DIALOGUE`, `ECON`, `MAGIC`, `COMBAT`, `EVENT`, `BACKEND` |

Foundation 本身拥有全局 vocabulary、跨系统 invariants、ID/时间/坐标基元、traceability 和文档索引，不计入十五个 subsystem。

`AI` 的 projection Schema 由 `AI` 自己发布为输入 Port；Application Orchestrator 查询其他 owner 的只读 Port、执行 Secret ACL 和 Revision 固定后，将字段复制到该 Schema。数据来源不产生 `AI -> source domain` import，`AI` 尤其不得 import `DIALOGUE`、`PLAYER`、`COMBAT`、`EVENT` 或其他调用方合约。
...
箭头表示 consumer 对 provider 的 direct import，每条边与第 5.1 节穷举集合一一对应；transitive dependency 不重复授权新的 direct import。图中 `DIALOGUE --> AI` 没有反向 `AI --> DIALOGUE`，AI 所需对话或世界数据由 Orchestrator 映射成 AI-owned projection。
...
| `AI` | 任一业务 domain 的 Query/Command/Event contract、任何 Repository、`DIALOGUE` 等调用方 import、未过滤 Secret、可信伤害/价格/路径计算 |
...
`DES-FOUNDATION-003`：一般跨域交互采用 `Query Port -> immutable DTO`、`Command Port -> Result` 或 `DomainEvent -> subscriber`；AI 调用是受限特例：Orchestrator 聚合 owner DTO 并映射为 AI-owned `DecisionContextProjection`，然后调用 AI Port，AI 不查询来源 domain。
...
AI 流程不执行第 1 步的 source-domain 查询：Orchestrator 完成查询、ACL 与 Revision 固定后传入 `DecisionContextProjection`；AI 只返回非权威 `ActionProposal`。
...
最小数据披露原则适用于 Domain Port：Orchestrator 只把当前 actor 可知字段映射进 AI projection，对话只接收其权限内字段。批量 Query 应提供按 Revision 缓存的只读 DTO，禁止 N+1 Repository 穿透；Domain Event subscriber 必须幂等并可重放。
...
| `TEST-FOUNDATION-015` | `RULE-FOUNDATION-010` | matrix 与 Mermaid direct edges 完全一致，Domain graph 无环，AI import 集合仅含 `FOUNDATION` |
