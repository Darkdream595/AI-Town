...
- `RULE-BACKEND-022`：统一验证顺序：transport → Origin/Host → 速率限制 → body 大小与 JSON 解析 → Session/CSRF → 权限 → `schema_version` 支持性 → payload Schema → 用例前置条件。任一步失败立即返回对应错误码，且不执行任何后续副作用。
...
`DES-BACKEND-004`：端点目录（auth 列含义见 `DOC-BACKEND-008`；所有 Schema 名称即注册表键，均含 `schema_version`）：

| 方法与路径 | 用途 | 请求 Schema | 响应 data Schema | Route Class |
|---|---|---|---|---|
| `GET /api/v1/health` | 进程健康与 Recovery Barrier 状态 | — | `HealthStatusV1` | health |
| `GET /api/v1/meta` | 应用版本、`protocol_version`、构建指纹 | — | `AppMetaV1` | health |
| `POST /api/v1/session` | 建立/刷新本地 Session Cookie | `SessionRequestV1` | `SessionInfoV1` | session |
| `POST /api/v1/ws-tickets` | 颁发单次 WebSocket Ticket | `WsTicketRequestV1` | `WsTicketV1` | ticket |
| `GET /api/v1/worlds` | 世界列表与元数据 | — | `WorldListV1` | world-admin |
| `POST /api/v1/worlds` | 创建世界（Seed、模板） | `WorldCreateV1` | `WorldSummaryV1` | world-admin |
| `GET /api/v1/worlds/{world_id}` | 单世界详情 | — | `WorldDetailV1` | world-admin |
| `POST /api/v1/worlds/{world_id}/open` | 打开世界（触发恢复序列） | `WorldOpenV1` | `WorldRuntimeStateV1` | world-admin |
| `POST /api/v1/worlds/{world_id}/close` | 关闭世界（Graceful Drain） | `WorldCloseV1` | `WorldRuntimeStateV1` | world-admin |
| `DELETE /api/v1/worlds/{world_id}` | 删除世界（需 Confirmation Token） | `WorldDeleteV1` | `WorldDeleteResultV1` | destructive |
| `GET /api/v1/worlds/{world_id}/saves` | 存档槽列表 | — | `SaveSlotListV1` | save |
| `POST /api/v1/worlds/{world_id}/saves` | 写手动存档槽（覆盖需 Token） | `SaveWriteV1` | `SaveSlotV1` | save |
| `POST /api/v1/worlds/{world_id}/saves/{save_id}/load` | 从槽位加载（分支语义见 `DOC-RELEASE-004`） | `SaveLoadV1` | `WorldRuntimeStateV1` | save |
| `GET /api/v1/settings` | 非敏感设置读取 | — | `SettingsV1` | settings |
| `PUT /api/v1/settings` | 非敏感设置写入 | `SettingsV1` | `SettingsV1` | settings |
| `PUT /api/v1/secrets/deepseek-api-key` | 提交/替换 DeepSeek Key | `SecretPutV1` | `SecretStatusV1` | secret |
| `GET /api/v1/secrets/deepseek-api-key/status` | Key 状态（masked） | — | `SecretStatusV1` | secret |
| `DELETE /api/v1/secrets/deepseek-api-key` | 删除 Key（需 Confirmation Token） | `SecretDeleteV1` | `SecretStatusV1` | destructive |
| `POST /api/v1/confirmations` | 颁发破坏性操作 Confirmation Token | `ConfirmationRequestV1` | `ConfirmationTokenV1` | destructive |
| `POST /api/v1/diagnostics/package` | 生成脱敏诊断包（Job） | `DiagnosticsRequestV1` | `JobResourceV1` | diagnostics |
| `GET /api/v1/diagnostics/jobs/{job_id}` | 轮询诊断 Job | — | `JobResourceV1` | diagnostics |
| `GET /api/v1/diagnostics/metrics` | 本地指标快照 | — | `MetricsSnapshotV1` | diagnostics |
...
1. 前端加载后依次调用 `GET /health`、`POST /session`、`GET /worlds`。
...
所有端点继承 `DOC-BACKEND-008` 的 Origin/Host、Session、CSRF、速率与 body 限制；`secret`、`destructive` Route Class 使用最严格限额并全量审计。响应不含文件系统绝对路径、堆栈或内部配置；`world_id`、`save_id` 等 ID 可出现。管理端点目标 p95 预算见 `DOC-BACKEND-012`。列表端点分页上限 100 条，防止超大响应。
...
| `TEST-BACKEND-014` | `RULE-BACKEND-021..022` envelope、版本与验证顺序无副作用 |
