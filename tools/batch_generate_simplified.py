...
PROMPTS_DIR = Path("docs/superpowers/assets/prompts_simplified")
...
def worker(task):
    ...
    print(f"[{idx}/{total}] {status} {filename} - {msg}")
...
def main():
    print("="*80)
    print(f"批量生成简化版 Sprite（{MAX_WORKERS} 并发）")
    print("="*80)
    ...
    # 收集所有提示词
    all_prompts = []
    for prompt_file in sorted(PROMPTS_DIR.glob("*_simplified.txt")):
